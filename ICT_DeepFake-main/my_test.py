...
class face_learner(object):
    ...
    def eval_all(self, conf, per='None'):
        ...
        eval_ds = fakeset(transform=self.eval_transform)
        sampler = None
        loader = DataLoader(eval_ds, batch_size=200, shuffle=(sampler is None), pin_memory=True, num_workers=8,
                                    sampler=sampler)
        self.evaluate(loader, query)
        del eval_ds
        del loader
    ...
    def evaluate(self, loader, query, nrof_folds=5, ):
        self.model.eval()
        idx = 0
        ...
        tpr, fpr, accuracy, best_thresholds, f1_score = evaluate_new(embeddings1, embeddings2, issame, nrof_folds)
        #binary_tpr = [1 if i > 0.5 else 0 for i in tpr]
        #conf_metrix = metrics.confusion_matrix(fpr, binary_tpr)
        #acc = metrics.accuracy_score(fpr, binary_tpr)
        #f1_score = metrics.f1_score(fpr, binary_tpr)
        #print("auc:",auc)
        #print("acc:",acc)
        ...
        for temp_rank in range(self.world_size):
            ...
            if self.rank == temp_rank:
                print("Rank{4} Evaluating {3} Acc:{0:.4f} Best_thres:{1:.4f} AUC:{2:.4f}".format(accuracy.mean(),
    ...
    def load_state(self, conf, save_path, fixed_str, model_only=False):
        ...
        if not model_only:
            ...
            self.head.load_state_dict(
                torch.load(os.path.join(save_path, 'head_{}'.format(fixed_str)), map_location='cpu'))
...
class a():
    def __init__(self):
        self.net_mode = 'ict_base'
        self.aug_test = 'store_true'
        self.dump_name = 'ICT_BASE'
        self.local_rank = 0
...
if __name__ == '__main__':
    ...
    if args.distributed:
        ...
        torch.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:' + port, rank=0,
                                             world_size=1)
