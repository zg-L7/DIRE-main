...
def binary_dataset(root: str, cfg: CONFIGCLASS):
    ...
    if cfg.isTrain:
        crop_func = transforms.RandomCrop(cfg.cropSize)#对图像进行随机裁剪，使得裁剪后的图像具有指定的大小
    else:
...
def get_dataset(cfg: CONFIGCLASS):
    ...
    return torch.utils.data.ConcatDataset(dset_lst)#将多个数据集对象连接在一起，创建一个包含所有数据的大数据集


def get_bal_sampler(dataset: torch.utils.data.ConcatDataset):
    """
    根据数据集中不同类别数量来设置权重读取数据集
    """
    targets = []
    for d in dataset.datasets:
        targets.extend(d.targets)#d.targets中的targets来自于ImageFloder返回的data.target,表示数据标签
