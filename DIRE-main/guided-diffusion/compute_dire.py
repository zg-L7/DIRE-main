...
def reshape_image(imgs: torch.Tensor, image_size: int) -> torch.Tensor:
    """
    给imgs添加一个表示批量的维度，将imgs图片大小调整为指定的image_size*image_size
    """
    if len(imgs.shape) == 3:
        imgs = imgs.unsqueeze(0)#原本imgs的shape为(通道数,宽,高),现在是添加了一个0维度的量,变为(batch_size,通道数,宽,高)
    if imgs.shape[2] != imgs.shape[3]:
        crop_func = transforms.CenterCrop(image_size)
        imgs = crop_func(imgs)
    if imgs.shape[2] != image_size:
        imgs = F.interpolate(imgs, size=(image_size, image_size), mode="bicubic")#调整尺寸为image_size
    return imgs
...
def main():
    ...
    dist_util.setup_dist(os.environ["CUDA_VISIBLE_DEVICES"])#配置分布式计算环境
    ...
    while have_finished_images < args.num_samples:
        ...
        for i in range(len(recons)):
            ...
            cv2.imwrite(f"{recons_save_dir}/{fn_save}", cv2.cvtColor(recons[i].astype(np.uint8), cv2.COLOR_RGB2BGR))#存储重建图像
...
def create_argparser():
    ...
    #使用model_and_diffusion_defaults()函数返回的字典来更新defaults的值
